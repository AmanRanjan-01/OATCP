...
signed main(){
    ...
    auto timeSt = std::chrono::high_resolution_clock::now();
    cout << ford_fulkerson(graph, source,sink) << "\n";
    auto timeEn = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(timeEn-timeSt);
    cout << duration.count()<<"\n";
    return 0;
}